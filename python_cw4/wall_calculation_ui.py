...
def get_wall_ui_frame(_root):
    ...
    def material_add():
        ...
        if len(i_item) == 0:
            ...
            list_material.insert(i_item,item)
            return
    ...
    def cal_wall_u(rair,rsi, rso, materials):
        r_total = rsi + rso+rair
        for m in materials:
            r_total += m[1] / m[2]
    ...
    def cal_u():
        ...
        if not is_numeric(rso):
            ...
            return
        if not is_numeric(rair):
            showerror(message='Air Space Layer Thermal Resistance Format Error')
        ...
        u = cal_wall_u(rair,rsi, rso, materials)
    ...
    label_rso = tk.Label(wall_frame, text="Outside Surface Thermal Resistance (in m²k/W): ")
    label_rsi = tk.Label(wall_frame, text="Inside Surface Thermal Resistance (in m²k/W): ")
    label_air = tk.Label(wall_frame, text="Air Space Layer Thermal Resistance (in m²k/W): ")
    ...
    input_air = tk.Entry(wall_frame, width=20)
    input_rso.insert(0,'0')
    input_rsi.insert(0,'0')
    ...
    btn_add = tk.Button(wall_frame, text="Insert New Material", command=material_add)
    ...
    label_air.grid(row=1, column=1)
    input_air.grid(row=1, column=2)
